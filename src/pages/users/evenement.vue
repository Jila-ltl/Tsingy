  ...
  <div class=" h-full flex flex-row">
    ...
    <div class=" w-[20%] h-full bg-lime-900 bg-gradient-to-t from-black/80 via-black/20 to-transparent pt-4">
      ...
      <div class="mt-10  ">
        <div
          ...
          :key="i"
          class="my-1 px-8 py-2 cursor-pointer hover:bg-stone-700"
          :class="item.isSelect?' bg-stone-800':''"
    ...
    <div class="w-[80%] max-h-[91vh] bg-white  overflow-y-auto">
      <div v-if="selectedEvent">
        <div class=" columns-1 md:columns-2 p-2 lg:columns-4 gap-3 space-y-2">
          <div v-for=" item , i in selectedEvent.imageData " :key="i" class="">
            <img alt="Event Image" class="w-full rounded" :src="item.url" @click="selecteItem(i,selectedEvent)">
          </div>
        </div>
      </div>
    </div>
    <div v-if="showDialog.show" class=" absolute z-50 top-0 left-0 w-full h-[100vh] flex items-center justify-center backdrop-blur-md" style=" background: #00000077;">
      <div class="relative group">
        <button class="absolute left-2 z-10 top-1/2 -translate-y-1/2 p-2 text-white hover:bg-white/10 rounded-full transition" @click="prevImage">
          <svg
            class="w-10 h-10"
            fill="none"
            stroke="currentColor"
        ...
        <img alt="" class="w-[80vw] h-[80vh] object-contain border border-white rounded-2xl bg-black" :src="showDialog.image[showDialog.index].url">
        <button class="absolute -top-0 -right-0 z-20 text-white text-2xl" @click="showDialog.show = false">✕</button>

        <button class="absolute right-4 z-10 top-1/2 -translate-y-1/2 p-2 text-white hover:bg-white/10 rounded-full transition" @click="nextImage">
          <svg
            class="w-10 h-10"
            fill="none"
            stroke="currentColor"
