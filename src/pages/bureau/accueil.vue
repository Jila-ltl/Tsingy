...
.reclamation-item {
    display: flex; /* Aligne la photo et le texte côte à côte */
    align-items: center;
    padding: 12px;
    cursor: pointer;
...
.card {
  ...
  max-width: 400px;
  border-radius: 24px;
  overflow: hidden; /* Pour que les bords soient arrondis en haut */
  background-color: #1a1a1a; /* Fond gris sombre des cadres */
  display: flex;
  flex-direction: column;
...
.bg-green { background: linear-gradient(135deg, #00c28e, #1a9e7a); }
.bg-yellow { background-color: #ffb31a; }
...
.overlay {
  ...
  backdrop-filter: blur(4px);            /* Effet de flou derrière (très moderne) */
  display: flex;
  justify-content: center;
  align-items: center;
...
.popup {
  ...
  box-shadow: 0 20px 40px rgba(0,0,0,0.3); /* Ombre portée douce */
  max-width: 600px;
...
.btn-close:hover {
  color: #000; /* Devient plus sombre au survol */
}
...
.fade-pop-leave-to {
  opacity: 0;
  transform: scale(0.9); /* Démarre un peu plus petit */
}
